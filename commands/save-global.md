...
When the user invokes `/afmem:save-global`, follow these steps:

### 1. Parse Arguments
Extract the key (first argument) and value (everything after the key).

### 2. Storage Location
Always use global storage: `~/memory/memories.json`

This memory will be available in ALL projects.

### 3. Ensure Directory Exists
Create the ~/.afmem directory if it doesn't exist:
...
if [ -f "$HOME/memory/memories.json" ]; then
  ...
  echo '{"memories":[]}' > "$HOME/memory/memories.json"
  echo '{"memories":[]}'
fi
```
...
Write the updated memories array back to `~/memory/memories.json` using the Write tool.

### 8. Confirm to User
Tell the user:
- Memory saved globally: `<key>`
- Storage location: ~/memory/memories.json
- Total global memories: N

## Example

User: `/afmem:save-global preferred-editor VS Code with Vim keybindings and Monokai theme`

Response:
```
Memory saved globally: preferred-editor
  Value: "VS Code with Vim keybindings and Monokai theme"
  Location: ~/memory/memories.json (global)
...
**Use global (`/afmem:save-global`) for:**
