...
When the user invokes `/afmem:save`, follow these steps:
...
- **Global storage**: `~/memory/memories.json` (cross-project memories)
- **Project storage**: `memory/memories.json` in current working directory (project-specific)
...
if [ -f "memory/memories.json" ]; then
  ...
  mkdir -p .afmem && echo '{}' > memory/memories.json
fi
```
...
Append to `memory/access.log`:
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] WRITE key=<key> agent=<agent_id> chars=<value_length>" >> memory/access.log
```
...
4. If a potential conflict is detected, append a warning to the output:
   ```
   ⚠️ Possible conflict with [other-key] — check /afmem:cortex resolve
   ```
...
**10a. Read all memories** from `memory/memories.json`
...
- If CLAUDE.md exists without AF Memory section: Append the generated content at the end
...
User: `/afmem:save user-prefers-bun User prefers Bun over npm for package management`
